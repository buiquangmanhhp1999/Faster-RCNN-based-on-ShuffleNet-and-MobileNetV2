...
# load data
# format of all_imgs: filepath, width, height, bboxes: list of {class, x1, x2, y1, y2}
...
for epoch_num in range(num_epochs):
    ...
    while True:
        try:
            ...
            # calculate the set of coordinates of boxes have shape (300, 4)
            R = roi_helper.rpn_to_roi(P_rpn[0], P_rpn[1], C, use_regr=True, overlap_thresh=0.7, max_boxes=300)

            # note: calc_iou converts from (x1,y1,x2,y2) to (x,y,w,h) format
            X2, Y1, Y2, IouS = roi_helper.calc_iou(R, img_data, C, class_mapping)
